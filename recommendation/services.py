...
class UserSessionService:
    def __init__(self, message_id: int, user: UserService) -> None:
        ...
        if not self.questions.count():
            self.add_session_questons()


    def add_session_questons(self) -> None:
        for category in self.categories:
            question = Question.objects.filter(category=category).order_by('?').get()
    ...
    def set_answer(self, session_question:SessionQuestion, text_answer: str) -> None:
